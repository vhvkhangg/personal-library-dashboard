...
type EditItemModalProps = { open: boolean; onClose: () => void; itemTitle: string; itemDescription?: string };
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className="focus-ring h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-sm text-[var(--foreground)] placeholder:text-[var(--muted)]" />;
}
function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className="focus-ring min-h-32 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-3 text-sm text-[var(--foreground)] placeholder:text-[var(--muted)]" />;
}
        ...
        {options.map((tag) => {
          ...
          return (
            <button
              key={tag}
              type="button"
              className={active ? "rounded-full bg-[image:var(--accent-active)] px-3 py-1.5 text-xs font-medium text-white" : "rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs font-medium text-[var(--foreground)] hover:bg-[var(--interactive-hover-bg)] hover:text-[var(--interactive-hover-text)]"}
              onClick={() => toggle(tag)}
            >
              {tag}
            </button>
...
export function EditItemModal({ open, onClose, itemTitle, itemDescription }: EditItemModalProps) {
  const [visibility, setVisibility] = React.useState("private");
  const [selectedTags, setSelectedTags] = React.useState(["Tag", "Theme", "Mood"]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="modal-scroll max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-[var(--border)] bg-[var(--card)] shadow-2xl shadow-black/40" onClick={(event) => event.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-[var(--border)] bg-[var(--card)]/95 px-6 py-5 backdrop-blur">
          <div className="flex flex-col items-start text-left">
            <p className="text-sm font-medium leading-none text-[var(--muted)]">Edit item</p>
            <h2 className="mt-2 text-[1.85rem] font-semibold leading-none tracking-tight">{itemTitle}</h2>
          </div>
          <button className="focus-ring rounded-xl p-2 text-[var(--muted)] hover:bg-[var(--interactive-hover-bg)] hover:text-[var(--interactive-hover-text)]" onClick={onClose}>
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-6 px-6 py-6">
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <Field label="Title"><Input defaultValue={itemTitle} /></Field>
            <Field label="Type"><Input defaultValue="Novel" /></Field>
            <Field label="Category / Context"><Input defaultValue="World" /></Field>
            <Field label="Rating"><Input defaultValue="8.5 / 10" /></Field>
          ...
          <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="space-y-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
              <Field label="Description"><TextArea defaultValue={itemDescription ?? "This placeholder description previews how a medium-length table description will look before opening View details."} /></Field>
              <Field label="Summary"><TextArea defaultValue="Placeholder summary for edit-state review with more room for module-specific details." /></Field>
              <Field label="Notes"><TextArea defaultValue="Placeholder notes, linked ideas, and metadata." /></Field>
            </div>
            <div className="space-y-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
              ...
              <button type="button" className="focus-ring w-full rounded-2xl border border-dashed border-[var(--border)] bg-[var(--card)] p-5 text-center transition hover:border-[var(--accent)] hover:bg-[var(--interactive-hover-bg)] hover:text-[var(--interactive-hover-text)]" onClick={() => setAttachmentOpen(true)}>
                <Upload className="mx-auto h-5 w-5 text-[var(--muted)]" />
                <p className="mt-3 text-sm font-medium">Replace preview or attachment</p>
                <p className="mt-1 text-xs text-[var(--muted)]">3 current attachments · click to preview or replace.</p>
              </button>
